...
SOFA (опц., только кандидаты) → локальный коммит
  │
  ▼
[ARCH] PRE-COMMIT GATE  (апрув = явное подтверждение завершённости)
  │
  ▼
COMMIT (статус итерации 🚧→✅, без push)
  │
  ▼
END
```
...
Апрув на этом gate — и есть «явный апрув завершённости» из `conventions.md` § Git → «Lifecycle итерации». Поэтому финальный коммит включает перевод статуса итерации 🚧→✅ Done в tasklist. Без апрува статус остаётся 🚧.
