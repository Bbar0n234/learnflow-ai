...
- `design-brief.md` — архитектор → planner и все роли. Несёт секцию `## Партиция треков` (идентификаторы треков итерации; заполняет оркестратор после входа архитектора, до PLAN).
...
- `sofa-proposals.md` — sofa-contributor → архитектор на апруве.
...
- `summary.md` — содержательно пишут implementer и fixer → tester/reviewers/docs-updater/sofa-contributor/harvester. Обязательные секции:
  ...
  - `## SOFA-посты (id / применил / результат)` — заводится структурно; наполняется в цикле фикса, читатель — sofa-contributor на финализации.
...
| Модель | Sonnet |
| Вход | `tracks/*/summary.md` (вкл. `## SOFA-посты`), `decisions.md`/ADR, `tracks/*/plan.md`, diff итерации, skill `sofa-contributor` |
