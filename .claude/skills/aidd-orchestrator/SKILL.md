...
|------|--------|-----------|
| planner | Opus | — |
| implementer | Sonnet | После 2 fail подряд (не прошёл `make check` или сабагент сам сообщил, что не справился) → перевызов с Opus |
| tester | Opus | — |
| code-reviewer | Opus | — |
| docs-updater | Sonnet | — |
...
3. Вызвать Agent tool: `subagent_type=general-purpose`, `model=<тир из таблицы>`, `prompt=<подставленный шаблон>`
