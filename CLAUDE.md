...
**Перед правкой кода в нетривиальной задаче открой `doc/tech/conventions.md`.** Жёсткие правила выше — краткая версия; conventions.md — источник истины по остальному (логирование, нейминг, типы, env-workflow, workflow миграций). Прочитай нужный раздел вместо того, чтобы гадать. Доменные конвенции (БД, API, agent runtime, frontend) вынесены в `doc/tech/conventions/` и подгружаются при работе в соответствующем домене — указатель в шапке conventions.md.
...
│   ├── conventions.md       # Git flow, нейминг, качество кода, документация (ядро)
