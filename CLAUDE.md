...
Обсуждая архитектуру, визуализируй компоненты, слои и потоки данных, чтобы архитектор мог дать качественную обратную связь, видя полную картину. Не жди, пока попросят. В чате — ASCII-диаграммы; в документах — Mermaid.
...
**Размещение worktree**: создавай worktree как соседнюю директорию основного репозитория, никогда — внутри него (например, `../learnflow-ai-<suffix>`, а не `.claude/worktrees/...`). Worktree, вложенный в репозиторий, слишком легко приводит к тому, что агент правит файлы в основном checkout вместо своего.
