...
- **🔐 Multi-tenancy Security** - API ключи и JWT авторизация для защиты пользовательских данных
...
**Преподаватели, репетиторы, методисты** — автоматическая генерация учебных материалов, тестов и вопросов для анализа знаний. Система обрабатывает как текстовые запросы, так и рукописные конспекты, создавая персонализированные обучающие материалы.
...
```bash
./local-logs.sh   # Просмотр логов всех сервисов
...
- **Полное покрытие** - Защищает вопросы, OCR-контент, HITL-обратную связь и запросы на редактирование
