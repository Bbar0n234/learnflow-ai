    ...
    messages = [
        {"role": "system", "content": RECOGNITION_PROMPT_TEMPLATE.render()},
        {
            ...
            "content": [
                ...
                *[{"type": "image_url", "image_url": {"url": f"data:image/png;base64,{img_data}"}} for img_data in image_list]
            ],
        },
    ]
...
def generate_additional_material(exam_question: str, client: OpenAI, prompts_config: dict) -> str:
    """
...
def synthesize_material(exam_question: str, lecture_notes: str, additional_material: str, 
                       client: OpenAI, prompts_config: dict) -> str:
    """
...
def main():
    ...
    parser.add_argument('--exam-question', default=default_exam_question, 
                       help='Экзаменационный вопрос')
    parser.add_argument('--images-folder', default='images/clipped', 
                       help='Папка с изображениями конспектов')
    parser.add_argument('--output', default='data/outputs/synthesized_material.md', 
    ...
    if not args.skip_generation:
        additional_material = generate_additional_material(args.exam_question, client, prompts_config)
    else:
        print("⏭️  Пропускаю этап генерации дополнительного материала")
        additional_material = "Дополнительный материал не генерировался (пропущено по запросу)"
