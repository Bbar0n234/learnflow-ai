...
def map_to_http_exception(exc: ArtifactsServiceException) -> HTTPException:
    ...
    if isinstance(exc, (ThreadNotFoundException, SessionNotFoundException, FileNotFoundException)):
        return HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=str(exc)
        )
    elif isinstance(exc, (InvalidPathException, FileTooBigException, TooManyFilesException, UnsupportedContentTypeException)):
        return HTTPException(
