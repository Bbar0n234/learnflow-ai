...
class ArtifactsStorage:
    """File storage system for LearnFlow AI artifacts."""
        ...
        if sessions_dir.exists():
            for session_dir in sessions_dir.iterdir():
                if session_dir.is_dir() and self.validate_session_id(session_dir.name):
                    try:
                        session_metadata = self.get_session_metadata(thread_id, session_dir.name)
                        sessions.append(SessionInfo(
                            ...
                            files_count=len(list(session_dir.rglob('*'))) if session_dir.exists() else 0
                        ))
    ...
    def get_session_files(self, thread_id: str, session_id: str) -> List[FileInfo]:
        ...
        if not session_path.exists():
            raise SessionNotFoundException(f"Session {session_id} not found in thread {thread_id}")
    ...
    def update_session_metadata(self, thread_id: str, session_id: str, metadata: SessionMetadata) -> bool:
    ...
    def read_file(self, thread_id: str, session_id: str, path: str) -> str:
        ...
        if not file_path.exists():
            raise FileNotFoundException(f"File {path} not found in session {session_id}")
        
        with open(file_path, 'r', encoding='utf-8') as f:
            return f.read()

    def write_file(self, thread_id: str, session_id: str, path: str, content: str, 
                   content_type: str = "text/markdown") -> bool:
        """Write file content."""
        # Validate content type
        if content_type not in settings.allowed_content_types:
            raise UnsupportedContentTypeException(f"Content type {content_type} not supported")
        
        # Check file size
        if len(content.encode('utf-8')) > settings.max_file_size:
            raise FileTooBigException(f"File size exceeds limit of {settings.max_file_size} bytes")
        
        # Check files count
        try:
            files = self.get_session_files(thread_id, session_id)
            if len(files) >= settings.max_files_per_thread:
                raise TooManyFilesException(f"Too many files in thread (max: {settings.max_files_per_thread})")
        except SessionNotFoundException:
