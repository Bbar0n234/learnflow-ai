...
are matched by their natural keys; chat messages are written to the
...
Artifact seeding (PG rows + a ``create_artifact`` tool turn in chat 1) was cut
in T1.7 along with ``ArtifactRepository`` (OQ-3: the PG artifact model is
gone, a file-backed replacement seeded into the workspace is a follow-up, not
this script's job today).
...
def _chat1_messages(thread_id: uuid.UUID) -> list[Any]:
    """Simple two-turn exchange, same shape as chat2.

    Pre-T1.7 this thread also seeded an inline PG artifact via a
    ``create_artifact`` tool-call/tool-result pair; that model is gone
    (OQ-3) and a file-backed replacement is a follow-up, not this script's
    job today — the turn is dropped rather than left pointing at a deleted
    repository.
    """
    return [
        HumanMessage(
        ...
        AIMessage(
            ...
            content=(
                ...
                "(`y' = f'(g(x)) · g'(x)`).\n\n"
                "Например, для `y = sin(x²)`: внешняя функция `f(u) = sin(u)` имеет "
                "производную `f'(u) = cos(u)`, внутренняя `g(x) = x²` — `g'(x) = 2x`. "
                "По правилу: `y' = cos(x²) · 2x = 2x·cos(x²)`."
            ),
...
async def seed_demo() -> int:
    ...
    async with (
        ...
        await graph.ainvoke(
            {"messages": _chat1_messages(chat1_id)},
