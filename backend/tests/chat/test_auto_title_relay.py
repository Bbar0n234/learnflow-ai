...
@pytest.mark.parametrize(
    ...
    [
        ...
        [trace_id_event("tr-1"), security_block_event("llm_classifier")],
    ],
    ids=["block_first", "block_after_consumed_trace_id"],
)
...
async def test_send_message_keeps_generation_when_block_arrives_later() -> None:
    ...
    events = await _run(
        _thread(),
        [text_chunk_event("Привет"), security_block_event("final_output")],
        title_generator=generator,
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (security_block_event("final_output"), ["text_chunk", "security_block"]),
    ],
