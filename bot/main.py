...
async def start_command(message: Message):
    ...
    welcome_text = ( # TODO: переформулировать с учётом актуальной специфики
    ...
    try:
        ...
        if pending_media and pending_media["photos"]:
            ...
            if not final_text:
                await message.answer(
                    telegramify_markdown.markdownify(
                        "❌ Пожалуйста, отправьте экзаменационный вопрос вместе с фотографиями." # TODO: переформулировать (непонятно какой формулировки придерживаться - "вопрос", "контент" или что0то ещё)
                    ),
                    parse_mode=ParseMode.MARKDOWN_V2,
                )
                return
