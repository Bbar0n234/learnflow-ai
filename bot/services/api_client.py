...
class LearnFlowAPIClient:
    ...
    async def update_hitl_config(self, user_id: int, config: HITLConfig) -> HITLConfig:
        ...
        try:
            ...
            response = await self._make_request(
                "PUT", f"/api/hitl/{thread_id}", config.to_dict()
            )
            return HITLConfig.from_dict(response)
    ...
    async def update_node_hitl(
        self, user_id: int, node_name: str, enabled: bool
    ) -> HITLConfig:
        """
        ...
        try:
            thread_id = str(user_id)
            response = await self._make_request(
                "PATCH", f"/api/hitl/{thread_id}/node/{node_name}", {"enabled": enabled}
            )
            return HITLConfig.from_dict(response)

        except Exception as e:
            logger.error(
                f"Failed to update node {node_name} to {enabled} for user {user_id}: {e}"
            )
            raise
    ...
    async def bulk_update_hitl(self, user_id: int, enable_all: bool) -> HITLConfig:
        ...
        try:
            thread_id = str(user_id)
            response = await self._make_request(
                "POST", f"/api/hitl/{thread_id}/bulk", {"enable_all": enable_all}
            )
            return HITLConfig.from_dict(response)

        except Exception as e:
            logger.error(
                f"Failed to bulk update HITL to {enable_all} for user {user_id}: {e}"
            )
            raise
