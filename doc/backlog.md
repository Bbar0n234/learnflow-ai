...
- **P2** Async / cloud agent workflow — поддержка запуска dev-агентов в облаке (disconnect-by-default), чтобы делегировать реализацию + тестирование и возвращаться к готовому PR. Стек: Claude Code (Managed Agents / web) и Codex Cloud как основные, Cursor Cloud Agent — точечно для UI-задач с visual evaluation. Состав:
  ...
  Связано со skill `aidd-orchestrator` (те же промпты работают и локально, и в cloud). Спаривается с Codebase-wide patterns & conventions pass в Tech Debt — вторая волна реализации идёт уже async-агентами по свежим конвенциям *(cross: Infra)*
