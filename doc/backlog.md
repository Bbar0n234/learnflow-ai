...
- **P2** Arch-docs style conventions + refactoring pilot — сформулировать принципы стиля арх-документации и применить к трём документам (`security/architecture.md`, `agent-runtime.md`, `auth.md`) через continuous improvement. Примерные направления: каждый концепт/англицизм при первом упоминании в документе — 1-2 строки "как это работает своими словами" + ссылка на источник правды, не просто термин; последовательность акцентов "вводные / контекст → компоненты → решения", не с деталей безопасности на пятой строчке; самодостаточность скоупа — для понимания документа X не должно требоваться лазить в >1 других документов. Результат: обновлённые правила в `aidd-methodology` skill, чек-лист для doc reviewer, три отрефакторенных документа. Итоговые формулировки правил появляются **по мере прохождения**, не заранее
- **P2** Tech/ full documentation audit — пройтись по всем документам в `tech/` через doc reviewer по принципам, откалиброванным в pilot выше. Выявить проблемные точки, завести точечные итерации на рефакторинг. Зависит от существования doc reviewer как формализованного агента (Workflow-agent prompts formalization)
