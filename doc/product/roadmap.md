...
flowchart TD
    ...
    P5a --> P5b["Фаза 5b · Product dogfooding"]
...
Готовлю первый контент на голом Claude Code (Opus), мимо продукта. Цель — не контент сам по себе, а **список требований** к продукту: какие агентные инструменты, какая обвязка, какие UX/память-моменты критичны. Каждое требование → триаж в backlog (помечается источником `[spike]`). Идёт **параллельно** достройке Трека B (брендинг) Фазы 4 — спайк ничего из Фазы 4 не требует.

Разделение сигналов: на Opus достоверно вылезают требования к **возможностям и workflow** (они от модели не зависят — нет инструмента, и любая модель не поможет); требования к **качеству** (структурирование, удержание рассуждения, чувствительность к промптам) на Opus непроверяемы и валидируются только в 5b на реальной модели. Паттерн ввода инструментов — [ADR-026](../tech/adr/ADR-026-tool-introduction-pattern.md).

Опора на Claude Code **затухает**, а не переключается разом: первый доклад — чистая разведка (~80% Claude Code), дальше каждый следующий начинаю в продукте, убегаю в Claude Code только где продукт не тянет — и каждый побег логирую как новое требование. Если 80% догфудинга остаётся в Claude Code — затея провалена; цель — большинство работы в продукте.

#### Фаза 5b — Product dogfooding

Полноценный догфудинг идёт **100% через дев-сервер** — не локально, не через Claude Code. Предусловие старта: выкатить все накопленные правки (дизайн и прочее, что висит в `develop` и не задеплоено) на дев-сервер, иначе брендинг и реальные косяки не вылезут. Будет больно — это и есть смысл фазы.

Скиллы всплывают по реальным потребностям: личный стиль/воркфлоу кристаллизуется в скиллы (как GOST-скилл), часть → обязательный bundle для всех пользователей, часть — project-specific или внешние. Генерация слайдов всплывает здесь как скилл / MCP. Конкретные скиллы заводятся в backlog по мере появления.
...
- **C1 — не-проектные, гейта нет, публикуются сейчас.** Это и есть контент Фазы 5a — работа не в стол: SOFA (опыт early adopter; скоропортится — статус и лидерборд обесцениваются, делать раньше), GOST-скилл (мелкий, без зависимостей — разогрев пайплайна), Security (флагман; блокирован фундаментальным red-team прогоном — backlog).
