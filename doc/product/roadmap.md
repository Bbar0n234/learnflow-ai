...
Инженерный трек Фазы 4 (Трек A) и discovery-спайк Фазы 5a завершены; активы спайка перенесены в продукт (многофайловые скиллы, генерация изображений, субагенты, skill-context). Накопленный `develop` ещё не задеплоен в `main`, догфудинг через продукт не начат. Ближайший путь — закрытие версии, деплой и Фаза 5b; план итераций до показа преподавателям — [tasklist-dogfooding](../tasks/tasklist-dogfooding.md).
...
Скиллы всплывают по реальным потребностям: личный стиль/воркфлоу кристаллизуется в скиллы, часть → обязательный bundle, часть — project-specific или внешние. План итераций фазы (инструменты догфудинга: file attachments, PDF-экспорт, слайды, модели; предпоказные фичи: OAuth, кастомные скиллы, web search, voice input) — [tasklist-dogfooding](../tasks/tasklist-dogfooding.md).
...
Все три предусловия закрываются раньше открытия фазы — итерациями [tasklist-dogfooding](../tasks/tasklist-dogfooding.md) до показа преподавателям (OAuth и web search — плановые итерации; cost-optimal модель подбирается по внешним бенчмаркам и валидируется догфудингом 5b).
