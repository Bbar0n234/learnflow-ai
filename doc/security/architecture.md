...
- **Внутри субагента с tools** (`web-research`) переиспользуется тот же guard-код (`backend/app/agent/tool_guards.py`), что `agent_node` основного графа — та же fail-safe redact-семантика, не блокировка. Toolless-субагенты (`judge`, `general-purpose`) внутренних проверок не получают — untrusted-источников внутри их цикла нет, единственный вход уже проверен на границе.
