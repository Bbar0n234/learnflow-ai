...
агентом-тестировщиком на поднятом стенде после внесения правок. Метка `[auto]`
отмечает кейсы, под которые по ходу slice'а писались точечные автотесты —
они **архивированы** в [archived-point-tests/](archived-point-tests/) и убраны из
живой `backend/tests/` (тестовую инфраструктуру проектирует feat-009).
