...
Статус: **кандидаты №1 и №2 ОПУБЛИКОВАНЫ** (с апрувом архитектора, агент `Bbar0n234`). Остальные —
черновики-предложения, не опубликованы. Публикация — внешнее действие под явным апрувом архитектора.

Опубликованные посты (тип TIL, актуальная версия v2):
- №1 — LangGraph dangling tool_call: `2123cfef-0c75-4e68-b188-f8498c39f744`
  → https://agents.stackoverflow.com/tils/2123cfef-0c75-4e68-b188-f8498c39f744
- №2 — CORS-on-500: `7138f19f-1bd2-41f9-9175-b18e547d46b0`
  → https://agents.stackoverflow.com/tils/7138f19f-1bd2-41f9-9175-b18e547d46b0

История публикации:
- v1 (id `4b7de429…` и `4be48aa6…`) — переписаны из шаблонной структуры (TL;DR/Environment/Root
  cause/Fix) в естественную прозу: площадка помечает шаблонность и засилье списков как AI-тэл.
  Поиск дублей по SOFA — чисто. В №2 при первом POST сработал link guardrail на `http://localhost`
  (422, host не в allowlist) — схема убрана.
- v2 (текущие) — добавлены код-сниппеты (фикс + repair-pass + repro-скелет для №1; before/after
  порядка middleware + TestClient-проверка для №2), репро переведено в нумерованные шаги (нумерация
  для реальной последовательности — нормальный паттерн, антипаттерн только у рефлекторного
  буллетинга прозы и одинаковых шаблонных скелетов). В №1 вставлена **verbatim-строка 400** от
  OpenAI-совместимого API (снята live минимальным malformed-запросом, не полным агент-прогоном) —
  это то, что ищут поиском. API SOFA не имеет эндпоинта правки → v2 = delete v1 + create заново
  (дедуп-скрин отверг бы v2 как near-identical, пока v1 жив; поэтому сначала delete, потом create).
...
|---|----------|-----|---------|--------|
| 1 | Висячий `tool_call` после исключения в tool-ноде навсегда ломает thread (LangGraph + OpenAI API) | TIL | ✅ **ОПУБЛИКОВАН** | Эмпирически воспроизведено на изолированном графе; нетривиальная интеракция checkpoint↔ReAct↔OpenAI-формат; бьёт любой LangGraph-агент с tools + чекпойнтером; фикс штатный, не воркэраунд. |
| 2 | Generic-500 handler снаружи `CORSMiddleware` → ответ 500 без CORS-заголовков | TIL | ✅ **ОПУБЛИКОВАН** | Классическая, но регулярно повторяемая ловушка порядка middleware в Starlette/FastAPI; проверено TestClient'ом для двух порядков; переносимо на любой FastAPI-сервис. |
| 3 | `statement_timeout`/`connect_timeout` в LangGraph Postgres saver/store — только через query-параметры libpq URL; psycopg3 vs asyncpg различаются | TIL | ❌ **НЕ ПУБЛИКУЕМ** (решение архитектора: недостаточно ценно) | Нетривиально: saver/store не принимают connection-kwargs, а два драйвера задают timeout по-разному (`options=-c ...` vs `server_settings`); ценно для всех на `AsyncPostgresSaver`. |
| 4 | Union-аннотация ответа (`JSONResponse \| dict`) ломает старт `create_app()` под FastAPI ≥0.135.1; фикс `response_model=None` | TIL | ❌ **НЕ ПУБЛИКУЕМ** (решение архитектора: недостаточно ценно) | Падение на старте, которое не ловят ruff+mypy; привязано к версии FastAPI; фикс прямой из текста ошибки. Узковато, но проверено и переносимо. |
...
Итог: **опубликованы 2 (№1, №2)**. №3 и №4 закрыты решением архитектора как недостаточно ценные;
№5 опциональный (не делали); №6 и доменные находки — не брать.
