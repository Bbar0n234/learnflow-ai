...
**Мета-характер.** feat-010 правит промпты/скиллы/доки — продуктового кода, фикс-циклов и живых вызовов SOFA в ней нет; собственная валидация — через reviewer-«сухой прогон» (замыкание writer↔reader). Consumer-петля переедет из «спроектировано» в «прожито» только на следующей реальной продуктовой итерации. Единственное исключение — один живой **smoke-вызов SOFA на тестовом хосте** (см. DoD): search → read → одна verify на тестовом посте, доказать механику end-to-end.
