...
**Остаток:** вкусовая полировка архитектору ({T4.8}/{E2E.6}) + предпрод-пункты (края cutout soft-balanced; стабы группы B с fake-данными — закрыты гейтингом ниже).

## Stub-gating: SHOW_GROUP_B_STUBS ✅

Пост-приёмочная правка (после апрува дизайна архитектором). Заглушки группы B рендерились в реальных вью с fake-данными **безусловно** — то есть уехали бы в прод и показались обычному пользователю (подтверждено grep'ом по `dist/`). Закрыто фиче-флагом `SHOW_GROUP_B_STUBS = import.meta.env.DEV` (`shared/config/feature-flags.ts`): в dev-сборке (`make dev-fe`) фичи видны для догфудинга, в прод-сборке флаг статически `false` → ветки не рендерятся, мок-данные выпадают из бандла через dead-code elimination.

Загейчены 8 точек рендера: peek-карточка в `MessageList`, studio-панель + линза в `ChatView`, чип «Студия» в `ChatHeader`, панель «Жизнь сферы» в `SphereView`, дропдаун «Сохранить версию» в `SphereViewer`, рейл истории версий в `SphereEditor`, диспатч вьюеров slides/image/audio в `ArtifactView` (в проде падает в дефолтный markdown-viewer), чип `v2.4.1 · растёт` в `ProjectLayout`, контрибьюшн-чипы в `ChatList`. Инвентаризацию вели сплошным свипом по маркерам — два инлайн-стаба (`ProjectLayout`, `ChatList`) были вне `model/mock-*.ts` файлов и в первичную инвентаризацию не попали. Верификация: `make check-fe` зелёный, прод-сборка собирается, распознаваемые мок-строки (`v2.4.1`, `Уточнён JTBD`, статистика сферы) физически отсутствуют в `dist/`.

Раз-гейтить по фичам по мере появления бэкенда (см. Backend-пункты группы B в backlog).

## Follow-ups

Канонический сток хвостов итерации для фазы HARVEST. Разбор кандидатов (backlog / конвенции / шум) — в `harvest-proposals.md`.

- **Гейтинг группы B → un-gate по бэкенду.** `SHOW_GROUP_B_STUBS` прячет незрелые фичи в проде; снимать индивидуально, когда у фичи появляется контракт. Триггеры — Backend-пункты backlog: версионирование сферы, SSE-peek, rich-артефакты. Предпрод-гейт (backlog P2) теперь закрыт в части «скрыть»; остаётся «свап на реальные данные».
- **Паттерн stub-gating — в конвенции.** Гейт незрелых фич за `import.meta.env.DEV` через единый флаг в `shared/config` — переиспользуемый приём (не только группа B). Кандидат в `conventions/frontend.md`.
- **Инвентаризация заглушек по свипу, не по grep мок-файлов.** Инлайн-стабы (хардкод в компонентах вне `model/mock-*.ts`) ускользают от поиска по именам констант. Урок для будущих фронт-итераций — known-trivial.
- **Недизайненные экраны и error/loading-UX** (auth, 404, SIEM-экраны, loading-скелетоны, per-query error) — уже занесены в backlog при сворачивании итерации.
- **Подход к тестированию/ревью фронтенда** (гид по ревью, Playwright e2e + visual-regression в CI, browser-инструмент фидбэка, догфудинг) — занесён в backlog (Agent Harness & Workflow, P1).
