...
| Что | Где | Действие |
|-----|-----|----------|
| Хелпер | новый, единая точка чтения клиентского IP | ветвление по `CLIENT_IP_SOURCE`; при `x-real-ip` / `x-forwarded-for` отсутствие или нехватка элементов → `socket` + WARNING (делает дрейф nginx видимым) |
...
| `CLIENT_IP_XFF_HOPS` | `int`, `1` | отступ справа; значим только при `CLIENT_IP_SOURCE=x-forwarded-for` |
...
2. Обновить `~/learnflow-ai/.env`: четыре новых переменных (в проде `CLIENT_IP_SOURCE=x-real-ip`) плюс пустой `COMPOSE_PROFILES`.
3. Убедиться, что nginx-конфиг соответствует референсной копии — в частности, что `proxy_set_header X-Real-IP $remote_addr` присутствует: от этого зависит корректность режима `x-real-ip`.
