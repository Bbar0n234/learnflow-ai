...
**Стенд финального прогона** — общий для cross-cutting кейсов всех четырёх треков, описан в
`../T1/test-cases.md` § Layer 2 (два прохода: dev-профиль с `COMPOSE_PROFILES=siem` и оба тумблера on,
прод-профиль с пустым профилем и обоими тумблерами off; порты переопределены переменными процесса,
боевой `.env` не читался и не правился).

- [x] **Полный стек на прод-образах поднимается и работает.** Кейс проверяет то, чего разовые `docker run` показать не могут: что урезанные образы действительно живут в связке — миграции применяются, сервисы стартуют, фронт отдаётся, межсервисный обмен цел. `docker compose` здесь уже разрешён: барьер INTEGRATION_TEST пройден, параллельных треков с testcontainers-БД больше нет.
  ...
  **Результат: pass, снят на обоих проходах стенда.** Dev-профиль (SIEM в игре): все пять контейнеров `Up (healthy)` — `app db redis siem-db siem-service`; в логах обоих сервисов миграции и старт uvicorn (`Running database migrations…` / `Running siem-service database migrations…`, `Application startup complete`, `Uvicorn running on http://0.0.0.0:8001`). Эндпоинты: `app /health` 200, `siem /health` 200, корень `/` (SPA из `frontend/dist`) 200. Межсервисный обмен цел: пять событий прошли путь `app` → Redis Stream (`XLEN` 8 → 13) → консьюмер siem-service (`event written to database` по каждому) → `siem_events` (11 строк) → REST `GET /api/security/events` под админским JWT 200 (`total: 12`). Прод-профиль на пересобранном образе: `app db redis` healthy, миграции применены, `/health` и `/` 200 (SIEM-часть неприменима — профиль пуст, это оговорённый в предусловии случай, а не пропуск). Состав окружения в **работающих** контейнерах: `import pytest` → exit 1 `ModuleNotFoundError` и в `app`, и в `siem-service` (то есть `uv run` при старте dev-группу не вернул), при этом `import uvicorn, alembic, fastapi, langgraph` → `runtime ok`. В логах старта обоих сервисов нет ни `Installed N packages`, ни имён dev-пакетов. `ModuleNotFoundError` на рантайм-зависимостях не встретился ни разу, включая живой диалог с агентом и запись артефакта в БД.
...
- [ ] 👤 **Приложение из прод-образа работает в браузере.** Открыть `http://localhost:${APP_PORT:-8000}` (SPA отдаётся backend-образом из `frontend/dist`, собранного стадией `frontend-build` того же Dockerfile), зарегистрироваться/залогиниться, отправить сообщение агенту и получить ответ. Кейс страхует от того, что урезанное окружение ломается не на старте, а на первом реальном запросе (SSE, LLM-вызов, запись в БД). **Провал**: 5xx, пустая страница, оборванный стрим при том, что стек по `{Layer 2}` считается поднятым.
  **👤 deferred (архитектор)** — браузерная проверка. Серверная половина риска («урезанное окружение ломается на первом реальном запросе») снята по HTTP на обоих профилях: SPA `/` отдаётся 200 из образа, регистрация/логин работают, SSE-диалог с живой моделью доходит до `done` с `trace_id`, а вызов тула `create_artifact` пишет артефакт в БД (`artifact_created`, `GET /projects/{id}/artifacts` → 1 запись). Ни одного 5xx и ни одного оборванного стрима за прогон.
