...
- **Слияние ветки feat-002 «Chat UX» и решение по триггеру auto-title (коммит `967598d`).** Две
  параллельные итерации догфудинга правили пересекающиеся файлы; feat-002 к этому моменту
  завершена и переписываться не будет — по решению архитектора адаптируется наша сторона, её
  функциональность сохраняется целиком. Разведено четыре конфликта. `api/routes/chats.py` —
  конфликт только в блоке импортов, обе группы ортогональны и оставлены обе (наш `_part_out()` +
  typed parts в `MessageOut` и их bodyless `create_chat` + `rename_chat`/`delete_chat`).
  `tests/chat/conftest.py` — конфликт в шапке словаря провода, объединён: наш словарь v2 плюс их
  `SERVICE_SYNTHESISED_TYPES = {done, title_updated}`, вся их title-инфраструктура сохранена без
  правок. `doc/tech/streaming.md` — их правка внесена по смыслу, а не склейкой: `title_updated`
  вошёл строкой в таблицу v2 и абзацем в «Уточнения» (с адаптацией под расширенный набор
  терминальных событий и под пролог потока), их описание relay-цикла влито в наш нумерованный
  список; попутно снята метапометка «будущий `title_updated`» в Forward-compat.
  Существенное решение сверх брифа пришлось на `services/chat.py`. У feat-002 генерация
  auto-title защищена инвариантом «заблокированный ввод не уходит в title-модель»: если первое
  событие потока — `security_block`, генерация не запускается. Проверка читала **литерально
  первое** событие, а в контракте v2 первым безусловно идёт `stream_started`, и до вердикта
  guard'а тишину заполняет `heartbeat` — дословный перенос их кода тихо сломал бы их же
  инвариант (генерация стартовала бы на прологе, до проверки ввода). Введён набор нейтральных
  типов пролога `_TITLE_GUARD_NEUTRAL_TYPES = {stream_started, heartbeat}`: решение принимается
  на первом событии **вне** пролога. Инвариант сохранён, поведение задокументировано в
  `streaming.md` § `title_updated`, тесты guard'а расширены двумя случаями пролога.
  Продолжение этой истории — следующий пункт: денилист над множеством, которым мы не управляем,
  протёк второй раз (не был учтён `cancelled` от пейсера), и предикат был перевёрнут на
  позитивный. Форма предиката, а не его содержимое, и оказалась первопричиной обоих промахов.
...
- **Фикс R3 (раунд 2): обе тихие fail-open-ветки внутри `execute_tools_guarded` стали громкими.**
  Ревьюер нашёл два пути, на которых батч результатов инструментов проходит узел `tools` без
  проверки и без следа в логе (`test-cases.md` § «Находки ревью, раунд 2», R3). Оба сегодня
  недостижимы, но открываются молча — а молчаливая деградация guard в этом проекте прямо
  запрещена: `conventions.md` § «Восстановление: fail-fast / graceful / fail-safe» требует, чтобы
  **все** дороги fail-open были одинаково наблюдаемы (решение D-ERR-6 итерации
  feat-007), и ровно этим требованием живёт сам движок (`security/guard.py`: две дороги
  деградации классификатора помечены `security_event=True` + `agent.guard.degraded` +
  `severity=critical` + `DetectionLayer.GRACEFUL_DEGRADATION`). Здесь были найдены третья и
  четвёртая. Первопричины у них разные, поэтому и починены они по-разному.
  Ветка «состояние не-dict» чинится по существу: проверка вообще не должна была зависеть от
  истории. `guard_tool_results` искала якорную `AIMessage` с `tool_calls` и на её отсутствии
  возвращала `[]` — то есть при нечитаемой истории (Pydantic/dataclass state schema) не
  проверяла **ничего**. Снят ранний `return` по `anchor < 0`: без якоря срез
  `messages[anchor + 1:]` естественно вырождается в «весь переданный батч», а
  `messages[:anchor + 1]` — в пустую историю. Классификатор теряет контекст диалога, но сам факт
  проверки не теряется никогда. Историю теперь достаёт `_state_messages` — dict и атрибут
  (`getattr`), то есть штатно поддержаны обе формы state schema; `None` (форма без читаемых
  `messages`) — сигнал деградации, а не разрешение пропустить проверку.
  Ветка «инструмент вернул `Command`» проверить батч честно не может: `ToolNode` в этом случае
  отвечает *списком* (`Command`'ы вперемешку с dict'ами на каждый обычный результат,
  `prebuilt/tool_node.py::_combine_tool_outputs`), и `ToolMessage`-батча, который можно отдать
  guard'у и заменить replace-by-id, в этой форме нет. Поддержать её по-настоящему — значит
  вводить обработку `Command` (в том числе запись редакций обратно в `Command.update`), то есть
  принимать архитектурное решение ради конструкции, которой в коде нет ни одной. Поэтому ветка
  остаётся fail-open, но перестаёт быть тихой: перед возвратом уходит
  `logger.error(security_event=True, event_type=agent.guard.degraded, severity=critical,
  metadata={checkpoint: tool_result, detection_layer: graceful_degradation, reason:
  unsupported_tools_output, output_type: ...})`. Появится первый `Command`-инструмент — узнаем из
  SIEM в первую же минуту, а не из инцидента. Обе записи идут через общий `_log_guard_degraded`
  той же формы, что и у движка, поэтому `event_type` взят существующий (`AGENT_GUARD_DEGRADED`) —
  словарь `siem-contracts` не расширялся, публичный контракт не менялся.
  Fail-secure (бросок исключения на неожиданной форме) сознательно не выбран: конвенция ставит
  security-пути в fail-safe/fail-open, а fail-secure — только под жёсткие требования, которых у
  этой итерации нет.

- **Фикс находки живого прогона: отчёт о результате инструмента стал повызовным (attempt 1).**
  Что было: предыдущий фикс перенёс проверку `TOOL_RESULT` внутрь узла `tools`, и вместе с
  проверкой туда переехала отчётность — но узел по-прежнему отвечал **батчем**. `ToolNode`
  исполняет все вызовы хода одним `ainvoke` и возвращается, когда закончился самый долгий из
  них, а `tool_result` снимался с апдейта узла, то есть в тот же момент для всех вызовов сразу.
  На живом стенде это дало: два `firecrawl_search`, реально отработавшие за ~10 с, простояли в
  ленте идущими и получили длительность **3:01**, потому что в одном батче с ними исполнялся
  `run_subagent`. Лента показала законченную работу как незаконченную и приписала ей чужое
  время.
  Первопричина по контракту — **единица отчётности не совпадала с единицей работы**: работа
  пользователя — это отдельный вызов инструмента (у него свой `call_id`, своя строка в ленте,
  своя длительность), а отчитывался узел батчем, потому что апдейт узла физически не может
  прийти раньше, чем узел вернётся. Никакой правкой момента чтения апдейта это не лечится —
  лечится только тем, что единицей исполнения и отчёта становится вызов.
  Фикс — в `tool_guards.execute_tools_guarded`:
  - `_split_per_call` режет батч на по одному входу узла на вызов (копия последней `AIMessage` с
    единственным `tool_call`, остальное состояние нетронуто — `ToolRuntime.state` видит то же,
    что видел бы при батче). Разбиение зеркалит `ToolNode._parse_input` (последняя `AIMessage`),
    поэтому набор исполняемых вызовов не меняется. Не режется: не-mapping state, нечитаемая
    история, батч из одного вызова — там резать нечего, идёт один вызов узла, как раньше.
  - вызовы по-прежнему идут параллельно (`asyncio.gather`), но guard и отчёт живут **внутри**
    каждой ветки: результат проверяется и уходит на провод, как только закончился свой вызов.
    `gather(return_exceptions=True)` — чтобы падение одной ветки не оставило соседей висеть
    неотожданными; первое исключение затем перебрасывается, как и раньше пробрасывалось из
    одного `ToolNode.ainvoke`.
  - `_merge_outputs` собирает ответ узла обратно в порядке, в котором вызовы просила модель (не
    в порядке завершения), — разговор в чекпоинте читается так же, как читался при батче.
  - `guard_tool_results` (батчевая, с поиском якорной `AIMessage`) заменена на
    `guard_tool_result` — «один результат, один вызов классификатора». Логика якоря ушла
    целиком: проверяются только те результаты, которые породил **этот** прогон узла, история —
    исключительно контекст для классификатора. Повторная проверка прошлых итераций стала
    структурно невозможной, а не «предотвращённой правильным индексом».
  Отчётность переехала с updates-канала в custom: `tool_result`/`artifact_created` теперь пишет
  сам узел `tools` (`stream_events.make_tool_result_reporter`, общий с субагентом — тот уже так
  работал с T1.6). На проводе типы, payload'ы и порядок не изменились —
  `tool_call_started → tool_call_args → tool_result`, вложенность по `parent_call_id`; изменился
  только момент и канал-источник. `StreamEventMapper` за узлом `tools` сохранил ровно реестр
  анонсированных вызовов (иначе `tool_call_cancelled` начал бы отменять уже разрешённые вызовы),
  но событий с этого узла больше не порождает — единственный эмитент результата теперь один.
  Осознанная цена, согласованная архитектором: классификатор зовётся по разу на результат вместо
  раза на батч — больше токенов и денег на параллельных вызовах. Обратно в батч не
  оптимизировать.
  Инварианты предыдущего фикса перепроверены поимённо: (1) на провод и в чекпоинт уходит только
  проверенный текст — проверка стоит перед `on_result` и перед возвратом узла, сторожит
  `test_poisoned_tool_result_never_reaches_the_wire_or_the_checkpoint` (теперь смотрит **оба**
  канала одного рана, а не только updates); (2) один вызов классификатора на результат — старые
  два кейса остались зелёными без правки ассертов (их обоснования переписаны: механизм «якорь»
  сменился на «проверяем только выход своего прогона»), сверху добавлен
  `test_every_result_of_a_parallel_batch_is_classified_exactly_once` — два вызова стоят два
  `TOOL_RESULT`, не четыре, и порядок результатов в ходе тот, что просила модель; (3) обе
  fail-open ветки и их наблюдаемость не тронуты — пять кейсов `test_tool_guards.py` зелёные без
  единой правки (нечитаемая история логируется один раз на узел, `Command`-форма — на ветку, где
  она возникла); (4) санированная ошибка субагентского инструмента — прежний путь, кейс
  `test_a_failing_subagent_tool_is_reported_as_an_error_result` зелёный; (5) порядок событий и
  вложенность — `test_a_tools_domain_event_still_reaches_the_custom_channel` усилен до полного
  списка custom-канала (доменное событие инструмента идёт **перед** `tool_result` своего
  вызова), субагентские кейсы `parent_call_id` не тронуты.
  Закрывающий кейс — `test_a_finished_call_is_reported_without_waiting_for_its_neighbour`
  (`tests/agent/test_graph.py`): реальный граф, медленный инструмент ждёт события теста, быстрый
  возвращается сразу; кейс снимает первое событие custom-канала и требует, чтобы это был
  `tool_result` быстрого вызова, пока медленный сосед доказуемо ещё внутри тула (`_slow_phases`
  без `"finished"`). Проверено мутацией дважды: (а) `_split_per_call` возвращает `None` всегда
  (батчевое исполнение) — 1 red / 21 green, красный ровно закрывающий; (б) отчёт перенесён с
  ветки вызова на смерженный выход узла — 3 red / 19 green (закрывающий плюс два кейса
  доменного события, которые ловят пропажу отчёта). Мутации откачены точечными правками.
  Тест-файлы, приведённые к новому контракту (не ослабление — перенос ассертов на новое место
  эмиссии): `tests/agent/test_stream_events.py` (кейсы payload'а `tool_result`/`artifact_created`
  переехали с `StreamEventMapper.updates()` на `tool_result_envelope`/`artifact_created_envelope`,
  плюс новый кейс «на payload узла `tools` маппер молчит» — иначе двойная эмиссия прошла бы
  незамеченной); `tests/image_generation/test_stream_events_generate_image.py` (то же);
  `tests/subagents/test_stream_isolation.py` (скриптованный апдейт заменён на custom-конверт —
  фильтр токенов субагента не должен глотать канал, по которому теперь идут результаты);
  `tests/chat/test_chat_service.py` — AST-страж словаря расширен: он сканировал только
  `StreamEvent(type=...)`, а половина словаря переехала в custom-конверты, и без объединения
  двух сканов страж остался бы «зелёным на пустом множестве».
  Документация: `streaming.md` (таблица — источник `tool_result`/`artifact_created`; § «`tool_result`
  / `artifact_created`» — повызовность, цена и почему updates-канал не годится; § «Вложенность
  субагента»; § «Каналы»; § «Security-чекпоинты» п. 4), `conventions/agent.md` и
  `agent-runtime.md` — строки про `StreamEventMapper` и `stream_mode`. Попутно поправлен дрейф
  имени `guard_tool_results` → `guard_tool_result` в докстрингах прода.
  Верификация: `make check` зелёный; `make test` — **935 passed / 1 failed**
  (`test_pricing_external.py`, внешний дрейф цен).

### Решения актуализации документации (docs-updater)

- **Заведён один ADR — [ADR-030](../../../../../../tech/adr/ADR-030-per-call-tool-result-guard.md)
  «Результат инструмента — проверка и отчёт повызовно, внутри узла `tools`».** Из трёх
  кандидатов, названных оркестратором, критериям роли (решение переживает итерацию, есть
  осознанный trade-off, есть отвергнутые альтернативы, влияет на будущие изменения) отвечает
  только этот. У него две силы в конфликте — inbound-граница защиты и правдивость ленты, —
  цена выражена в деньгах (классификатор на каждый результат вместо батча) и явный запрет
  «обратно в батч не оптимизировать», который без ADR первый же оптимизатор снимет как
  случайную неэффективность. Архитектурные документы после этого ссылаются на ADR за
  обоснованием и не повторяют его: `streaming.md` описывает механику, `security/architecture.md`
  — место чекпоинта, `agent-runtime.md` — узел графа. В `doc/index.md` ADR внесён строкой в
  курируемую выборку — она собрана из решений, определяющих поведение системы, и это одно из
  них; без строки в навигации его нашёл бы только тот, кто уже знает, что ищет.
- **Два других кандидата ADR не заведены, и вот почему.** *Приоритет явно переданного writer'а
  над ambient при вложенности субагента* — не развилка с альтернативами, а единственный рабочий
  вариант: вложенный граф исполняется `ainvoke`, его собственный custom-стрим никто не читает,
  что проверено экспериментально и записано прямо в `streaming.md` § «Вложенность субагента»
  вместе с отвергнутым `subgraphs=True`. ADR добавил бы второе место с тем же содержанием.
  *Позитивный предикат вместо денилиста в защите над множеством, которым мы не управляем* — не
  архитектурное решение, а норма кодирования: её место в `conventions.md`, а не в ADR. Сам
  случай и его причина зафиксированы в `streaming.md` § `title_updated` (почему предикат
  позитивный) и в § «Решения и обоснования» выше, так что знание не теряется; вопрос о
  возведении её в конвенцию вынесен архитектору — заводить проектную норму своей властью роль
  не может.
- **Нового архитектурного документа не создавалось.** Прогнал по критериям: контракт SSE v2
  кросс-сервисен и объёмен, но у него уже есть свой документ (`streaming.md`), и итерация его
  переписала, а не переросла. Лента активности — единственный кандидат на выделение, но она
  живёт в двух документах по разным вопросам (`frontend.md` — что за компоненты, `conventions/
  frontend.md` — раскладка состояния и правила живости) и своей иерархии подсекций не
  наработала; отдельный документ породил бы третье место, которое надо синхронизировать.
- **Метапометки сняты, в том числе спорная.** `conventions/agent.md` — чек-лист больше не
  привязан к артефакту итерации; `streaming.md` — снято «не исправлено этой ревизией
  документа» при сохранённой констатации факта. Формулировку «удалены относительно предыдущей
  версии контракта» (вопрос ревьюера, а не нарушение) переписал на утверждение о текущем
  состоянии: имена `tool_start`/`tool_end` в тексте остались — они нужны читателю, который
  ищет их в документе и в старом клиентском коде, — но сравнение с прошлым ушло.
- **Дрейф, поправленный на месте в `security/architecture.md`** (сверено по коду, не по
  summary): утверждение «checkpointer хранит оригинал, заглушку подставляет DTO-mapper» было
  верно только для `tool_call_arg` — у `tool_result` редакция теперь заменяет сообщение в узле,
  и сырой текст не доезжает ни до провода, ни до чекпоинта, у `final_output` подмена идёт по
  `id`; описание разведено по трём чекпоинтам. Добавлен абзац о наблюдаемости деградации в
  enforcement-адаптере (обе fail-open ветки узла `tools` пишут `agent.guard.degraded`) — без
  него каталог событий знает про эти дороги, а карта защиты нет. Снята метапометка «graduated
  response — feat-007» у SUSPICIOUS (проверено: отдельной реакции в коде нет) и номер итерации
  у SIEM-pipeline.
- **`streaming.md` § «Frontend: потребление стрима» приведён к факту:** утверждение «диспетчер
  не падает на неизвестном типе, но и не логирует его» устарело — forward-compat реализован с
  логированием. Добавлен пункт «живой ход и история рисуются одной моделью» — это протокольное
  свойство контракта (эквивалентность событий и `parts`), а не деталь фронта, поэтому оно
  здесь, а раскладка состояния осталась ссылкой на `conventions/frontend.md`.
