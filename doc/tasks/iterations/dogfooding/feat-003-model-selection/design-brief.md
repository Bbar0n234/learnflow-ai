...
2. **External, постоянный контур** (marker `external`, входят в дефолтный `make test`): (а) price-drift — цены `pricing.yaml` (input/output/input_cache_read) против живого `GET https://openrouter.ai/api/v1/models`, расхождение → fail с диффом; (б) доступность whitelist — каждая whitelist-модель существует в каталоге, поддерживает tools, контекст ≥ 100k. **Семантика падений**: OpenRouter недоступен (timeout, 5xx, сетевая ошибка) → `pytest.skip` с причиной; данные расходятся → fail. Marker регистрируется в `backend/pyproject.toml` (`external: hits third-party network APIs`).
