...
| Серия инцидентов s.jina.ai в августе 2026 (error rate до 88% на несколько часов) | аптайм 90-дневный при этом 99.98% | риск НЕ закрыт полностью: инструменты Jina объявлены в `subagents.registry`, и при недоступности сервера на старте `_validate_subagent_tool_pool` валит boot fail-fast (graceful degradation работает только для инструментов вне субагентских спек). Известная проблема, зафиксирована в backlog (P2 «Graceful-деградация старта при недоступном built-in MCP») |
