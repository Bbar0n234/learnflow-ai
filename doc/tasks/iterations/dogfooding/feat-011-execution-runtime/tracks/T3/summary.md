...
### Добор по аудиту (пакет B): аутентификация executor'а и guard песочницы

Аудит нашёл две дыры в модели доверия. Первая: у executor'а не было никакой аутентификации — доступ держался только на сетевом сегменте `exec`, хотя ADR-031 сравнивает его с PostgreSQL, у которого пароль есть. Компрометация контейнера `app` давала произвольное выполнение кода плюс rw ко **всем** workspace (`project_id` — обычное поле запроса). Вторая: `EXECUTOR_SANDBOX_ENABLED=false` снимает bwrap целиком, а защитой был только WARNING на вызов. Обе правки одобрены архитектором на pre-commit gate.

**B1 — shared secret.** `POST /jobs` требует `Authorization: Bearer <EXECUTOR_AUTH_TOKEN>`; проверка живёт в новом `services/executor/executor/api/auth.py` и подвешена на сам `APIRouter` (`dependencies=[...]`), а не на handler — любой будущий роут этого роутера закрыт по умолчанию, забыть нельзя. `GET /health` объявлен на app, вне роутера, и остаётся открытым: его опрашивает healthcheck compose. Сравнение — `hmac.compare_digest`; отказ — `401` + `WWW-Authenticate: Bearer` и WARNING с `reason=missing_credentials`/`token_mismatch` (в сам ответ разница не протекает).

*Почему Bearer, а не свой заголовок.* В репозитории нет ни одного кастомного auth-заголовка, зато Bearer говорят все machine-to-machine вызовы: `app/api/deps.py`, `HTTPBearer` в siem-service, исходящий `Authorization` MCP-клиента. Своё имя заголовка было бы третьим соглашением на ровном месте; `HTTPBearer(auto_error=False)` вдобавок даёт разбор схемы и OpenAPI-описание бесплатно, а `auto_error=False` — потому что встроенный отказ отдаёт 403 без `WWW-Authenticate` и без лога.

*Политика отсутствия токена — fail-fast, эталон `JWT_SECRET`.* Поле `auth_token` в `Settings` executor'а и `executor_auth_token` в `Settings` backend'а — **без дефолта**; в `docker-compose.yml` переменная пробрасывается обоим сервисам как `${EXECUTOR_AUTH_TOKEN:?…}`. Так требует conventions.md § Секреты и fail-fast, и здесь это не формальность: любой дефолт (включая `""` с «если пусто — не проверяем») означает, что деплой поднимется, пройдёт healthcheck и будет выглядеть рабочим, оставив `/jobs` открытым для всего, что дотянулось до порта, — ровно тот сценарий тихой деградации, который аудит и просил закрыть. Цена для dev — одна строка в `.env` (файл и так обязателен из-за `JWT_SECRET`); `.env.example`/`.env.local.example` обновлены, CI копирует `.env.example` в `.env`, тестовые харнессы подставляют значение сами (`backend/tests/conftest.py` — `os.environ.setdefault` рядом с `JWT_SECRET`; в executor'е — `services/executor/tests/__init__.py`, потому что `executor.main` строит `create_app()` на импорте, и переменная нужна раньше, чем импортируется conftest).

*Backend-проводка (файлы T1).* `Settings.executor_auth_token` (`backend/app/config.py`) и заголовок в `app/infra/executor.py`; `EXECUTOR_AUTH_TOKEN` добавлен в `.env.example`, `.env.local.example` и в оба сервиса `docker-compose.yml`. Отказ авторизации не растворяется: на `401/403` клиент пишет ERROR «значения `EXECUTOR_AUTH_TOKEN` двух концов должны совпадать» и дальше отдаёт обычный `HTTPStatusError` — то есть агент видит привычную ошибку инструмента, а не «runtime unavailable» (маскировать конфигурационную ошибку под аварию инфраструктуры было бы хуже: агент начал бы «чинить» код), а оператор получает причину, а не молчание.

**B2 — выключенная песочница стала громкой.** `create_app()` при `sandbox_enabled=False` пишет **ERROR** на старте («джобы исполняются без изоляции, с rw ко всем workspace»), `GET /health` отдаёт `{"status": "ok", "sandbox": "enabled"|"disabled"}` — деградация наблюдаема снаружи контейнера, а не только в логах. WARNING на каждую джобу в `build_job_argv` **оставлен**: он покрывает другой момент времени (кто-то мог перезапустить сервис и не посмотреть на старт), удалять уже работающую и покрытую тестами сигнализацию ради «одного места» смысла нет.

*Hard-guard «не стартовать вне dev» не реализован — осознанно.* Ни в `Settings` backend'а, ни в `Settings` executor'а нет маркера окружения (`app_env`/`environment`/подобного); единственное похожее имя, `LANGFUSE_TRACING_ENVIRONMENT`, — knob SDK Langfuse, читаемый не нами и не про топологию. Заводить новую env-переменную ради одного guard'а — архитектурное решение вне скоупа добора, поэтому зафиксировано как кандидат в backlog: **fail-fast старта при выключенной песочнице вне dev требует общего маркера среды**; появится маркер — guard добавляется в `create_app()` одной проверкой.

**Тесты.** Executor: 14 новых кейсов — `tests/api/test_auth.py` (9: без заголовка, неверный токен, три формы кривого `Authorization`, положительный случай, открытый `/health`, лог с `reason`, «отклонённая джоба не доходит до раннера» — маркерный файл не создаётся), `tests/config/test_settings.py` (2: обязательность и чтение `EXECUTOR_AUTH_TOKEN`), `tests/smoke/test_app_boot.py` (3: `sandbox: disabled` в `/health`, ERROR на старте, тишина на нормальном пути). Стартовые логи читаются из stdout, а не через `capture_logs`: `create_app()` вызывает `configure_logging()`, который переконфигурирует structlog и сносит перехватчик. Backend: 4 кейса в `tests/workspace/test_executor_client.py` (Settings без токена → `ValidationError`; заголовок реально уходит; ERROR на 401 и на 403). Итог — executor 101 passed (было 87), `backend/tests` 1231 passed.

**Мутационная проверка B1** (мутация копированием файла, откат копированием — не git): снятие проверки в `require_service_token` (тело → `return`) красит ровно 7 негативных кейсов `test_auth.py` и ничего больше (94 passed); снятие заголовка в `app/infra/executor.py` красит ровно `test_run_job_presents_the_shared_secret`. Оба файла восстановлены, суиты снова зелёные.

**Гейты:** `make check` (ruff + ruff format + mypy backend/siem/executor/tools + import-linter 9/9 + arch-checker — только унаследованные size-warning'и) зелёный; executor 101 passed; `backend/tests` 1231 passed (красный только `tests/agent/test_pricing_external.py` — известный внешний дрейф цен, см. Follow-ups); `make test-contracts` 66 passed; `docker compose config` валиден и раздаёт одно значение обоим сервисам, а без переменной падает внятным «EXECUTOR_AUTH_TOKEN is required — set it in .env». Живая связка app↔executor не проверялась по указанию оркестратора (стек перезапускается из-за установки gVisor).

**Побочный эффект в воротах релиза образа.** `make smoke-executor` запускает сценарии внутри контейнера, и `smoke_sandbox.py` строит `Settings()` — обязательное поле без дефолта сломало бы ворота, хотя сама смоук-цепочка ходит в раннер напрямую, без HTTP и без auth-барьера. Поэтому `docker run` в цели получил `-e EXECUTOR_AUTH_TOKEN=smoke-suite-placeholder` с комментарием, зачем он там. Проверено `make -n`; полный прогон `make smoke-executor` требует пересборки образа (`make docker-build-executor`) — на этом хосте не гонялся, стек занят установкой gVisor.

**Правки дрейфа документации, вызванного этим изменением:** `doc/tech/executor.md` (новый § «Аутентификация вызывающего», строка knob-таблицы, контракт `POST /jobs` и его sequence-диаграмма, переписанный буллет dev-фолбэка, `auth.py` в структуре модулей), `doc/tech/backend.md` (строка `EXECUTOR_AUTH_TOKEN` в конфигурации), `doc/tech/setup/production.md` (раздел про executor утверждал, что *ни одна* `EXECUTOR_*` не требует правки в боевом `.env` — теперь одна требует; добавлен способ сгенерировать значение), `doc/tech/setup/codex-cloud.md` (минимальный `.env` docker-less режима). **Операционное:** в локальный `.env` этого хоста переменная добавлена (случайные 32 байта hex) — иначе `docker compose up` на dev-машине больше не поднимется; на прод-VM это шаг чек-листа деплоя.
