...
- [x] `B1` **пройден** на живой топологии (`make docker-up`). `docker compose exec executor env` — весь env сервиса это ровно семь `EXECUTOR_*` knobs плюс `GPG_KEY` базового образа python; точечная проба `printenv` по шести именам (`DATABASE_URL`, `JWT_SECRET`, `LLM_API_KEY`, `FIRECRAWL_API_KEY`, `LANGFUSE_SECRET_KEY`, `REDIS_URL`) даёт пустую строку на каждое. Секреты основного стека до сервиса не доезжают — compose передаёт executor'у только его собственный префикс.
- [x] `B2` 🔴 **пройден**. Джоба (`code`, `socket.create_connection` с таймаутом 5 с) по всем трём адресам стековой сети получает `gaierror [Errno -3] Temporary failure in name resolution`: `db:5432`, `redis:6379`, `app:8000`. Отказ приходит на резолве имени — executor вне `default`-сети, DNS compose ему недоступен, то есть ни маршрута, ни имени.
- [x] `B3` 🔴 **пройден**, обе половины. Сырой IP мимо DNS (`1.1.1.1:443`) → `OSError [Errno 101] Network is unreachable` — это пустой netns самой джобы, а не отсутствие резолва; `urlopen("https://example.com")` → `URLError` по имени; `pip install requests` → две retry-попытки и `ERROR: No matching distribution found for requests`. Наружу из джобы не уходит ничего.
- [!] `B5` **пройден частично — runsc-половина deferred prod.** `docker inspect` контейнера: порт наружу **не опубликован** (`8002/tcp:<nil>` — expose без publish), `user: 10001:10001` (внутри `id` → `uid=10001(executor)`, non-root), единственная сеть — `..._exec`, `PidsLimit=256`, `Memory=2 GiB`, `NanoCpus=2`, `StopTimeout=320`. `Runtime=runc`, **не** `runsc`: на этом dev-хосте gVisor в демоне не зарегистрирован и `.env` несёт dev-override `EXECUTOR_RUNTIME=runc` (известное ограничение среды, см. `{T3.5}`); прод-дефолт в compose остаётся `runsc`. Заодно подтверждено, что фикс R11 держится в compose-топологии, а не только в ручном `docker run`: `PID 1` контейнера — `tini`.
- [x] `B6c` 🔴 **пройден**. Директорию `/workspaces/proj-int/artifacts` создал **образ app** под своим uid (`docker compose run --rm app` → `uid=10001(app)`, дерево `app:app`), после чего джоба executor'а (`uid=10001(executor)` — то же числовое значение, другое имя в своём образе) записала в неё файл и прочитала обратно: `exit_code=0`, `cwd=/workspace`, `-rw-r--r-- executor executor job-wrote.txt`. Единый uid межтрекового контракта 4 подтверждён на общем volume — ровно тот случай, который под gVisor дал бы `EINVAL` при расхождении.
- [!] `B8` **executor-половина пройдена, агентская — заблокирована** (нет живого хода, см. сводку ниже). Прямо через `POST /jobs`: бесконечный цикл (`while True: pass`, deadline 5 с) → возврат за **5.15 с** wall-clock, `exit_code=-15`, в stderr `[executor] job exceeded timeout of 5.0s — killed (SIGTERM, then SIGKILL after 5s grace)`. Fork-bomb (500 фоновых `sleep`, deadline 8 с) → упирается именно в `pids_limit` контейнера (`bash: fork: retry: Resource temporarily unavailable` ×4), убита по deadline, `exit_code=-15`; executor сразу после неё отвечает `/health` → `{"status":"ok"}`, хост цел. runs: r1 [!] (executor-половина) → r2 ✅ (закрыта агентская). На живом ходе: «запусти скрипт с бесконечным циклом» → джоба убита по backend-deadline (JSON-лог executor'а `duration_ms=60005, timed_out=true, exit_code=-15`), диагностика доехала до агента текстом `exit_code: -15` + `[executor] job exceeded timeout of 60.0s — killed (SIGTERM, then SIGKILL after 5s grace)`, агент разобрал её верно (объяснил SIGTERM/SIGKILL и двухфазное завершение), чат остался живым и принял следующие сообщения.
- [x] `B9` — **пройден**. runs: r1 ⚠️ (нет `app`) → r2 ✅. «Стоп» нажат на 15-й секунде джобы `sleep 50 → artifacts/lecture-1/orphan.txt`: ход остановлен («Выполнял код · не завершён», «Генерация остановлена»), композер разблокирован. Джоба, как и предписано отменой v1, дожила до конца и записала файл — `orphan.txt` появился в списке артефактов как сирота (в ленте чата его нет), призраков в списке нет, список актуален. Следующее сообщение в этом чате отработало штатно.
- [x] 📊 `stop_grace_period` ≥ deadline — **пройден**. Джоба `sleep 30; echo … > artifacts/grace.txt` запущена, через ~8 с в неё прилетел `docker compose stop executor`. Стоп занял **16.2 с** (столько оставалось джобе), контейнер вышел с `ExitCode=143` — то есть uvicorn дождался in-flight запроса и завершился штатно, а не был дорублен SIGKILL'ом. Джоба вернула вызывающему `{"stdout":"DONE\n","exit_code":0}`, файл `grace.txt` лежит на томе целиком (`survived-graceful-stop`, 23 байта). Запись на полпути не оборвана.
- [x] 📊 backend-deadline ≤ `EXECUTOR_MAX_TIMEOUT_SECONDS` — **пройден**. runs: r1 [!] (только инвариант конфигурации) → r2 ✅. В резолвленном compose `EXECUTOR_JOB_TIMEOUT_SECONDS=60` при `EXECUTOR_MAX_TIMEOUT_SECONDS=300` (запас клиента `EXECUTOR_CLIENT_TIMEOUT_GRACE_SECONDS=10`), то есть рабочий deadline впятеро ниже потолка. На живом ходе это подтвердилось эмпирикой: бесконечный цикл убит на `duration_ms=60005`, то есть по backend-deadline, а не по потолку сервиса; событий `timeout clamped` в логах executor'а за весь прогон (≈25 джоб, включая два намеренных таймаута) — **0**.
- [x] 📊 Langfuse-спан джобы (`exit_code`, длительность) — **пройден**. runs: r1 ⚠️ (нет `app`) → r2 ✅. На каждый вызов рождается SPAN с именем инструмента (`execute_code` / `run_command`), в `metadata.duration_ms` — длительность джобы, в `output` — `{"exit_code": …}`. Выборка за прогон: `30131 / exit_code 0`, `60041 / exit_code -15` (убитая по deadline), `183 / exit_code 1` (упавший скрипт), `69 / exit_code 0`. Спаны у джоб, до которых executor не дожил (сервис остановлен) и у отменённой пользователем, приходят без `duration_ms`/`exit_code` — результата джобы у них и нет. Рядом лежит TOOL-observation с сырым `inputs` и текстом результата, который увидел агент.

### Layer 3: E2E (cross-cutting, в INTEGRATION_TEST)

- [x] `A1` 👤 — **пройден**. runs: r1 ⚠️ → r2 ✅. Агент сам собрал данные и посчитал по ним: `write_file · artifacts/data/marks.csv` (20 строк `student,score`), затем `execute_code` с pandas → ответ числами (среднее 78.20, медиана 77.5, ст. отклонение 10.61). Лента показывает и вызов, и результат; разворот строки даёт сырой код и stdout. Второй раз то же звено сработало на 17-МБ csv из вложения: `run_command · wc -l uploads/big15.csv` → «900001».
- [x] `A2` 👤 — **пройден**. runs: r1 ⚠️ (только уровень образа) → r2 ✅. Вопрос «какая версия pandoc в окружении» → строка ленты «Выполнил команду · pandoc --version», ответ агента из stdout джобы: «pandoc 2.17.1.1» с расшифровкой сборки (pandoc-types 1.22.2.1, texmath 0.12.4, …). Звено «агент → tool → ответ из stdout» закрыто.
- [x] `A10` 👤 — **пройден**. runs: r1 ⚠️ → r2 ✅. Намеренно падающий скрипт (`10 / 0`) вернулся агенту как обычный результат инструмента: `exit_code: 1` + полный traceback `ZeroDivisionError: division by zero` в stderr. Агент прочитал stderr, объяснил, что песочница перехватила исключение и не упала сама, и починил код проверкой делителя; тред остался живым. Границу «ошибка джобы ≠ отказ инфраструктуры» модель держит — сравнить с `{T1.7}`, где на `execution runtime unavailable` она, наоборот, отказалась править код.
- [x] `A12` 👤 — **пройден**. runs: r1 ⚠️ → r2 ✅. PDF, загруженный вложением через `POST /uploads`, извлечён агентом в песочнице (`execute_code` + `pypdf`) и использован дальше по ходу — пересказ лекции, затем конспект в `lecture-1/notes.md`. Подробности и оговорка про PDF без текстового слоя — в `tracks/T2/test-cases.md`, Layer 3.

**Сводка cross-cutting прогона T3 (INTEGRATION_TEST, заход 2, 2026-08-12):** 14 passed, 0 failed,
1 частично (`B5` — runsc-половина остаётся **deferred prod**: на dev-хосте runsc не зарегистрирован
в демоне, `docker info` знает только `runc`). Блокер захода 1 снят merge'ем `develop`, и все шесть
заблокированных кейсов (`B9`, Langfuse-спан, `A1`, `A2`, `A10`, `A12`) плюс обе половинчатые позиции
(`B8`, backend-deadline) закрыты на живых ходах агента. Регрессий по треку T3 не вскрыто; фикс R11
(`tini`) держится и на длинном прогоне — ноль зомби после ≈25 джоб, включая три убитые по deadline
и одну отменённую пользователем.

*Заход 1 (история): 6 passed, 3 частично, 6 заблокировано, 0 failed — единственной причиной
блокировок был не стартовавший `app` (fail-fast `_validate_subagent_tool_pool` на
`unknown tool name(s) firecrawl_extract`, 32 рестарта); причина внешняя, снята в develop.*
