...
- **Фикс по code review (F2, nit обеих рецензий): Firefox-фолбэк доведён до вложенных контейнеров.** `scrollbar-width` не наследуется — объявленный только на `:root`, он давал тонкую полосу одному корневому скроллеру, а ~19 вложенных `overflow-auto` оставались с дефолтной шириной (цвет доезжал, потому что `scrollbar-color` наследуется). Принятым расхождением брифа было «10px против thin», а фактически выходило «стиль есть против стиля нет» — другое расхождение, не то, что утверждали. Внутри того же `@supports not selector(::-webkit-scrollbar)` правило разделено: `scrollbar-width: thin` на `*`, `scrollbar-color` остаётся на `:root`. Мокап вешает фолбэк на сам скролл-контейнер (`.scroll-new`) — охват теперь тот же, но глобальным правилом, как требует бриф § 1.2 («без точечных классов по контейнерам»). Замер в Firefox по-прежнему не делался — движок в среде недоступен.
- **Фикс по code review (F4, nit review-b): `ErrorCard` больше не рисует сырой `<button>`.** Единый шаблон состояний заводился в том числе против сырых кнопок (бриф § 2 называет это дефектом `ErrorBoundary`), а фундаментный `ErrorCard` вернул сырой элемент в `shared/ui` — и размножил его по семи потребителям. Кнопка «Повторить» переведена на `Button variant="link" size="sm"`: фокус-кольцо, `disabled`-семантика и переходы приезжают из примитива. Вид сверен с мокапом (`.err-card .retry`, ui-polish.html:313-314) — цвет `--destructive`, постоянное подчёркивание с offset 3px, hover через прозрачность, вес 500; поэтому `text-primary`/`hover:underline` варианта перекрываются классом (`h-auto px-0 text-sm text-destructive underline underline-offset-[3px] hover:opacity-80`), а не остаются от примитива. Наблюдаемый контракт (роль `button` + имя) не изменился — тесты потребителей зелёные.
- **Фикс по code review (F5, nit review-a): `LoadingState` получил `role="status"`.** Спиннер `aria-hidden`, подпись обычным `<span>` — единственная форма загрузки продукта (панели, Suspense, гарды, ~10 мест) не объявляла себя живой областью, и переход «загружаем → загрузили» скринридером не озвучивался вовсе. Роль на корне блока, спиннер остаётся `aria-hidden`, — как в `StreamEndNotice.tsx` и как у «Сохранено» в настройках проекта.
...
### Docs-updater: актуализация документации по итогам итерации

Обновлено по всем десяти пунктам `review-b.md` § «Незамеченный дрейф документации»:

- **`doc/tech/auth.md`** — правило исключений refresh-интерцептора переписано с префиксного (`!url.includes("/auth/")`) на семантическое (`CREDENTIAL_ENDPOINTS`: эндпоинты без access-токена), с причиной (баг «сайдбар без футера») и правилом классификации для будущих OAuth-эндпоинтов feat-008. Диаграмма после `flowchart TD` не тронута — её входной узел общий («Ошибка 401 от API»), правило исключения живёт в прозе до диаграммы, конфликта нет.
- **`doc/tech/frontend.md`** — сигнатуры действий `stream-store` (`applyEvent(ownerChatId, event)` и т.д. + новый `reset()`), owner-guard описан как инвариант стора, а не дисциплина вызывающего; таблица маршрутов — добавлен catch-all `*` и намеренная несимметрия `/security`; Module Structure (диаграмма + ASCII-дерево + текст «Принципы») — добавлены `pages/not-found`, `features/auth` (с пометкой «0 потребителей»), новые файлы `shared/ui` (`StateScreen`/`LoadingState`/`ErrorCard`/`skeleton`, `AuthLayout`/`ProviderButton`), ранее отсутствовавший в дереве `shared/assets/`; § Error UX и Error Boundary переписаны под `StateScreen`/`LoadingState`/`ErrorCard`/`Skeleton` со ссылкой на `design-system.md`.
- **`doc/tech/conventions/frontend.md`** — § «Состояние стрима» получил два новых абзаца: owner-guard на всех четырёх охраняемых действиях стора + отдельная семантика `reset()`, и контракт владельца в колбэках `useAgentStream` (`onDone.chatId` / `ownerChatId` первым позиционным аргументом во всех ветках, включая не-SSE). Критерий заведения слайса в `features/` дополнен `chat-actions` (был в дереве, не был в списке критерия) и осознанным исключением `features/auth`.
- **`doc/tech/design-system.md`** — добавлен `--success` в таблицу бренд-токенов с явным разграничением от `--mcp-connected`; новый подраздел «Скроллбары и `color-scheme`» (10px webkit-стиль на `--border`, паритет с `ScrollArea`, Firefox-фолбэк, `scrollbar-gutter: stable`); § Иллюстрации — девять сцен (было четыре названо) + таблица канонических ширин подачи; § Error UX переписан вокруг системы `StateScreen`/`LoadingState`/`ErrorCard`/`Skeleton` с инвариантом расстановки (списки — скелетоны, панели/страницы — `LoadingState`, per-query ошибки — инлайн, тосты — только мутации).

**Пункт 10 (граница под фирменные цвета провайдеров) — документ не менялся.** Развилка была в пользу токенов, не border-исключения: `harvest-proposals.md` фиксирует, что кандидат на новую границу в `design-system.md` § Границы снят на code review — существующая формулировка («значение вне темы заводится CSS-переменной, а не инлайн-литералом») уже покрывает `--provider-*` без правок, код (`ProviderButton.tsx`) приведён к ней. Ничего дописывать не пришлось.

**Побочный фикс дрейфа (не из списка ревью, найден по ходу).** `design-system.md` § Темизация утверждал, что `Illustration` и обёртка `sonner` подписаны на `theme-store` через селектор — по коду (`shared/lib/use-theme.ts`) оба читают тему `useSyncExternalStore`-хуком над DOM-классом `.dark`, специально в обход `stores` (граница FSD `shared → shared`). Тот же дрейф отмечен в `## Follow-ups` этого summary до правки автором T1.2 — исправлено здесь же, при проходе по соседнему разделу. Драйф старше этой итерации (код `use-theme.ts` не менялся в `git diff develop...HEAD`).

**Новый документ — не заводился.** Система состояний (`StateScreen`/`LoadingState`/`ErrorCard`/`Skeleton`) не проходит критерии `workflow.md` шаг 4.2: не кросс-сервисная (чистый frontend), укладывается в один подраздел `design-system.md` § Error UX без потери читаемости, не несёт отдельного жизненного цикла от дизайн-системы в целом. Расширенный контракт (полные сигнатуры пропов, рецепты скелетонов) уже задокументирован в JSDoc `StateScreen.tsx` и в «Решения и обоснования» T1.4 этого файла — второе место архитектурного уровня детализации ему не нужно.

**ADR — не заводился, предложение вынесено оркестратору.** Владелец потока в контракте `useAgentStream` + owner-guard в `stream-store` — решение полноценное (закрывает конкурентный баг, вводит паттерн для будущих писателей стора), но узкое по охвату: один hook + один store одного слайса, паттерн уже явно объяснён в `conventions/frontend.md` § «Состояние стрима» с полным «почему». Действующие ADR проекта (`doc/tech/adr/`) фиксируют решения уровня подсистем (топология сервисов, security-архитектура, хранилище памяти) — этот случай на них не похож по весу. Рекомендация: ADR не заводить, но решение — не докс-агента, выносится архитектору отдельным пунктом.

**Навигация (`doc/index.md`).** Не менялась — новых документов не появилось, у всех обновлённых файлов записи в навигации уже были.

## Follow-ups

- ~~**Дрейф `doc/tech/design-system.md`** (§ Темизация, строка 95; § Иллюстрации, строка 110)~~ — **исправлено фазой DOC_UPDATE**: `Illustration`/`sonner` описаны как читающие тему shared-хуком `useTheme` через DOM, а не подпиской на `theme-store` (см. «Решения и обоснования» выше, «Побочный фикс дрейфа»).
