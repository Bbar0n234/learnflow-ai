...
- **Фикс по code review (F1, blocker review-b): брендовые цвета переведены на токены `--provider-*` — два решения выше отменены.** Резолюция оркестратора 1, разрешившая литералы «как фирменные цвета чужого бренда», признана ошибочной: `design-system.md` § Границы разбирает ровно этот случай и даёт другой ответ — «когда значение должно жить вне темы… оно всё равно заводится CSS-переменной (`--slides-*`, `--scrim-overlay`), а не инлайн-литералом, чтобы граница „нет hex в компонентах“ оставалась абсолютной». Поэтому семь литералов уехали в `index.css` (`--provider-google-{blue,green,yellow,red}`, `--provider-github-fg`, `--provider-yandex-{bg,fg,stroke}`) рядом со `--slides-*`, а иконки красятся через `style={{ fill: "var(--provider-…)" }}` — тем же приёмом, что `SlidesViewer` и `SphereLens`. Значения не менялись (сверены с мокапом), включая обе темы знака GitHub: тёмный оттенок теперь не `dark:fill-[…]`-вариантом в `.tsx`, а переопределением `--provider-github-fg` в `.dark`, то есть ветвление по теме ушло туда, где живут все остальные ветвления. Отдельно: `stroke="rgba(0,0,0,0.14)"` под исключение «чужой бренд» не подпадал вовсе — это наша декоративная обводка белого круга, и токен ей нужен тем более. Landing-решение по § Границы (документировать исключение) отпадает: код приведён к норме, документ менять не надо.
- **Фикс по code review (F6, nit review-a): подпись сабмита во время отправки.** Было `{submitting ? "…" : SUBMIT_LABELS[mode]}` — доступное имя кнопки вырождалось в многоточие, и скринридер объявлял «кнопка, многоточие». Заведён `SUBMITTING_LABELS` (`Входим…` / `Создаём аккаунт…`) плюс `aria-busy`: имя остаётся осмысленным, а состояние сообщается отдельным каналом. Приём тот же, что на соседнем экране настроек проекта («Удаляем…»), — критерий внутри итерации сведён к одному. Меняет наблюдаемый контракт: `LoginScreenView.test.tsx` ищет сабмит по имени `"…"` (`OWN_FORM_BUTTON_NAMES`, `:68`, `:267`) — тест не правился, кейс передан автору тестов.
- **Фикс по code review (F7, nit review-b): неразрывный пробел в подписи Яндекса.** `DEFAULT_LABELS.yandex` теперь несёт U+00A0 между «Яндекс» и «ID», как в мокапе (`Войти с Яндекс&nbsp;ID`, ui-polish.html:927/931): ниже `lg` карточка тянется во всю ширину, и «ID» могло уехать на вторую строку в отрыве от названия. Меняет наблюдаемый контракт — доступное имя кнопки перестаёт совпадать с литералом обычного пробела в тестах (`ProviderButton.test.tsx`, `LoginScreenView.test.tsx`); ассерты не правились, кейсы перечислены в отчёте.
