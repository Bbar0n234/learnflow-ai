...
**WIP под ревью архитектора. Ничего не опубликовано и не отправлено.** Это терминальная точка
автономной роли `sofa-contributor` (planned-work, шаги 1–2): анализ артефактов итерации → ранжированные
кандидаты. Публикация постов и отправка write-back (verify/vote/reply) — только под явным апрувом
архитектора (SKILL § Author gate), после dedup-поиска на площадке (шаг 4).
