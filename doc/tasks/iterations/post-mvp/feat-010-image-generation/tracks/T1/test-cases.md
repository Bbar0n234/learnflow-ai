...
- [ ] **Агент → артефакт+блоб → media endpoint end-to-end.** Через реальный чат-стрим агент вызывает `generate_image`; по SSE приходят `tool_start(generate_image)` → `tool_end` → `artifact_created`; затем `GET /projects/{pid}/artifacts/{aid}/media` под JWT отдаёт те же байты с `image/*` mime и immutable-кэшем. (T1 ∥ T2 барьер; cost/ключ — как в `{T1.1}`.)

### Layer 3: E2E (cross-cutting, в INTEGRATION_TEST)

- [ ] **Полный journey: генерация → карточка-превью → вьюер.** 👤 Пользователь просит обложку → в ленте плейсхолдер (шиммер) на время генерации → по `artifact_created` карточка с превью 64×40 → открытие артефакта показывает реальную картинку, caption = prompt, скачивание `.png`. (Кросс-трек T1+T2; браузер.)
