...
Итерация доработок по ревью PR #80 (решения архитектора): toolless-форма графа
удалена — любой субагент теперь ReAct-агент с обязательным непустым `tools`
(boot-инвариант в `_validate_subagent_tool_pool`), judge и general-purpose получили
firecrawl-пул (judge — для выборочного фактчека); `sync_prompts.py` стал полностью
registry-driven (имена из `prompts.yaml`, write-back `model`/`extra_body` с защитой
от расхождений, без холостых пере-дампов `agent.yaml`); в конвейер оркестратора
добавлена фаза PLAN_REVIEW и сверки с design-brief у reviewer-b/test-reviewer;
size-чек (500 строк / 10 модулей) добавлен в arch-checker warning-only; в
conventions зафиксированы двухуровневая норма исключений, правило ревью брифа перед
финализацией и слоистость security «движок vs enforcement». Все прогоны зелёные
(679 backend + 31 subagents). Детали — блок «Итерация доработок» в § «Решения и
обоснования».

Вторая волна доработок (по итогам E2E-приёмки архитектора на стенде): UX-фикс
«молчаливого стрима» — мгновенный индикатор «агент думает» (`ThinkingIndicator`) +
first-byte-таймаут 30с→300с (диагноз: заголовки SSE-ответа задерживает pre-stream
guard-проверка, 30с ложно обрывали легитимные прогоны; из-за этого же аборта был
потерян ответ ассистента — ран отменился до коммита чекпоинта); наблюдаемость —
debug-логи реплик (`user message`/`agent reply`, превью 500 символов) и честный
`status=client_disconnected` вместо ложного `ok` при разрыве клиента;
`SUBAGENT_RECURSION_LIMIT` вынесен из кода в `agent.yaml` (`subagents.recursion_limit`,
дефолт 10). Мокап вариантов live-индикатора (V1/V2/V3, обе темы) — `mockups/
live-feedback-variants.html`; выбран V1 (реализован), V2/V3 — под зонтичный
backlog-пункт интерактивности. Harvest landing выполнен (5 пунктов в backlog, включая
продуктовую развилку «модель безопасности субагентов» вместо узкого A1; fail-fast с
агрегацией — пример в conventions). SOFA-контур исполнен: опубликованы Blueprint
subagent-as-tool (`6a673759…`) и TIL про `subgraphs=False` (`a997323d…`), write-back
по всем 4 консультированным постам отправлен (verify×2, vote↑×3, reply×1, vote↓×1 —
по явному апруву). Прогоны финальные: `make check` зелёный, 32 subagents-теста,
152 frontend-теста.
...
- **Итерация доработок по ревью PR #80 — решения архитектора по комментариям.**
  (1) **Единый ReAct-граф**: тезис «general-purpose без tools и single-turn —
  вырожденный агент, которым не будут пользоваться» → инвариант «любой субагент —
  ReAct-агент; ноль tool-вызовов — вырожденный случай прогона, не отдельная форма».
  `_build_toolless_graph` удалён, один builder, `bind_tools` безусловный (пустой
  `tools: []` в API-запросе провайдер-зависимо отвергается — потому пустой toolset
  запрещён на уровне конфига, а не обходится в коде); «spec.tools непуст» — второй
  класс нарушений в fail-fast `_validate_subagent_tool_pool`. Judge получил
  firecrawl-инструменты для выборочного фактчека (ревью может требовать проверки
  фактов), general-purpose — как полноценный универсальный исполнитель.
  (2) **`tool_guards.py` остаётся в `app/agent/`**: это enforcement-адаптер при
  движке `app.agent.security`, по прецеденту `runtime_security.py`; слоистость
  зафиксирована в docstring и `conventions/agent.md`, рефактор
  `security/enforcement/` — кандидат в backlog. (3) **`UnknownSubagentTypeError`
  легализован** двухуровневой нормой conventions § Модель ошибок (внутреннее
  исключение подсистемы, гасимое до барьера) — закрывает nit B2 ревью.
  (4) **`sync_prompts.py`** переписан registry-driven — закрывает комментарий
  архитектора и harvest-кандидатов «пробел sync_prompts» + P3. (5) **Конвейер**:
  PLAN_REVIEW (план ↔ бриф, до имплементации, ≤2 цикла) + бриф во входах
  reviewer-b/test-reviewer — закрытие дыры «после PLAN бриф читал только
  test-author». (6) **Ревью брифа перед финализацией** — правило процесса в
  conventions (рядом с Blueprint-ресёрч), вне FSM оркестратора. (7) **Size-чек**
  warning-only в arch-checker (2 файла > 500 строк, 4 директории > 10 модулей на
  момент включения); перевод в fail — после итерации рефакторинга нарушителей.

- **Вторая волна доработок — находки E2E-приёмки архитектора.**
  (1) **Таймаут SSE и потеря ответа**: «первый байт» в `useAgentStream` — это
  заголовки fetch-ответа, а их задерживает pre-stream guard (20–30с даже на
  тривиальном запросе) → 30-секундный лимит ложно обрывал прогоны; сам аборт
  клиента отменял LangGraph-ран (граф драйвится потреблением SSE-генератора) в
  окне до коммита чекпоинта — сгенерированный ответ терялся, а `finally`
  рапортовал ложный `status=ok`. Клиентский минимум закрыт (индикатор + 300с +
  `client_disconnected`); развязка графа от SSE-отдачи — backlog P2 (осознанно
  отложена: правка стрим-ядра, скоуп итерации не расширяем). (2) **Логи реплик**:
  INFO-тишина — норма конвенции (контент → DEBUG, полный контент → Langfuse);
  пробел был в незадействованном DEBUG — закрыт двумя `logger.debug`.
  (3) **`recursion_limit` → конфиг** (`subagents.recursion_limit`): прямое
  решение архитектора «операционные константы — в agent.yaml»; других кандидатов
  на вынос не найдено (остальные константы контрактные, у основного графа —
  неявный дефолт LangGraph 25). (4) **Продуктовые решения в backlog** по
  наблюдениям стенда: FP/TP императивных tool-инструкций, редактирование/версии
  артефактов, зонт интерактивности вывода агента, reply-шаг SOFA-пайплайна,
  развилка безопасности субагентов (вместо узкого A1).
