...
- [x] Layers & abstractions diagrams — по согласованному принципу «карта сервисов в `vision.md`, слои сервиса в документе сервиса»: `vision.md` (общесистемная, добавлен SIEM-контур), `backend.md` (актуализирована: реальные директории, `models/`, `security_pipeline/`, composition root), `frontend.md` (добавлена, as-is со всеми отступлениями от FSD), `doc/tech/siem-service.md` (новый полный документ: топология, pipeline, корреляция, persistence, API, конфигурация; секция в backend.md сжата до ссылки). Все — Mermaid, без `fill:`, рендер проверен на тёмной теме.
