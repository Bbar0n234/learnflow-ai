...
| feat-012 | M | ✅ Done | agent | ГОСТ-скилл: bundle-скилл оформления студенческих работ по ГОСТ 7.32 (.docx) — оффер для студенческой волны |
...
| feat-014 | O | 📋 Planned | backend | Стоп-кран затрат: быстрый ограничитель расхода LLM-бюджета на открытом проде (регистрация/лимиты) — до полноценного per-user cost budget |

## Порядок и приоритеты

```
Сделано    A/B/C/D/I/J/L/M/N закрыты; накопленный develop влит в main и задеплоен (PR #98)
Авг W3     feat-006 (G, слайды — топ-1: spike локально → скилл → deps в образ executor)
           feat-005 (F, PDF + UI артефактов — топ-2: экспорт, вьюеры PDF/DOCX, баги отображения)
           feat-014 (O, стоп-кран затрат — топ-3: быстрый ограничитель на открытом проде)
           → ПОКАЗ ПРЕПОДАВАТЕЛЯМ
Далее      админка / менеджмент-консоль оператора (backlog § Backend, после стоп-крана)
           студенческая волна (оффер ГОСТ; гейт — полноценный потолок затрат, backlog Backend)
           feat-007 (H) ── feat-010 (K) ── maintenance [dogfood]
```

- **Минимум к показу:** остались F и G — решение архитектора: показ с полным набором выходных форматов. M (ГОСТ-скилл) закрыт по факту переноса во время feat-011. H (кастомные скиллы), K (voice) — за гейтом показа.
- **Приоритет пересмотрен архитектором (2026-08-15):** G → F (+UI артефактов) → O (стоп-кран). Мотив O: регистрация на проде открыта, домен публичен и индексируется, ограничений расхода LLM-бюджета нет — риск не гейт студенческой волны, а текущий. Полноценный per-user cost budget — отдельно, ближе к реальному продакшну (сентябрь–октябрь), backlog § Backend.
- **Догфудинг идёт параллельно итерациям**; находки — в backlog с пометкой `[dogfood]`, разбор в maintenance-режиме.
...
### feat-005 (F): PDF-экспорт + UI артефактов

**Цель:** рабочий PDF-экспорт артефактов с рендером формул и фирменным стилем — преподавательский материал должен выходить файлом. Расширена архитектором (2026-08-15): сюда же — приведение в порядок отображения артефактов во вьюере (предпросмотр документных форматов + баги вёрстки), чтобы выходные форматы не только собирались, но и прилично показывались.

**Статус:** 📋 Planned
**Scope:** backend (cross: Frontend, Design-branding)
...
- **P2** Документные артефакты без предпросмотра во вьюере — файловая модель относит `.pdf` и `.docx` к категории `binary`, вьюер показывает заглушку «Предпросмотр недоступен» с кнопкой «Скачать» (media/download-эндпоинты сами файлы отдают корректно). Execution runtime сделал оба формата рядовым выходом агента — «скачай и открой снаружи» на своём же результате читается как незаконченный продукт. PDF дешёвый: категория `pdf` в `shared/lib/artifact-category.ts` + `PdfViewer` через `<object>`/`<iframe>` с blob-URL по механике авторизованной загрузки media у `ImageViewer`. DOCX: клиентский рендер (`mammoth.js` → HTML, теряет часть оформления) либо серверная конвертация в PDF (нужен LibreOffice в образе) — решение на design-brief *(Frontend, cross: Backend, выявлено при догфудинге feat-011)*

#### Новые элементы (наблюдения архитектора с прода, 2026-08-15; детализация и воспроизведение — на design-brief)

- Отображение image-артефактов: неудобный просмотр, ощущение, что картинки обрезаются — пересмотреть композицию вьюера.
- Вёрстка пустого/binary-артефакта едет: заглушка «предпросмотр недоступен» плохо отцентрирована, линии/рамки дорисованы не до конца — UI-баг.
...
Закрыта по факту, без отдельной итерации: скилл перенесён и проверен по горячим следам во время feat-011 (execution runtime) — воспроизведение отчёта по ГОСТу на живом стеке сработало. Апрув закрытия — архитектор, 2026-08-15. Остаток вынесен в backlog: профили титульников «вуз × тип работы» и решение по авто-СОДЕРЖАНИЮ (§ Agent); дистрибуционная часть (оффер студенческой волне) и так жила в backlog § Product / Distribution.
...
### feat-014 (O): Стоп-кран затрат

**Цель:** быстрый ограничитель расхода LLM-бюджета на открытом проде — защита от «чужак зарегистрировался и выжигает токены», пока полноценного per-user cost budget нет. Мотив (решение архитектора, 2026-08-15): регистрация открыта, домен публичен и индексируется — риск текущий, а не гейт студенческой волны.

**Статус:** 📋 Planned
**Scope:** backend (cross: Agent, Security)

#### Рамка (детализация — на design-brief)

- Сознательно «стоп-кран», не фундамент: дёшево, быстро, обратимо — в духе операционных kill-switch'ей ([ADR-029](../tech/adr/ADR-029-operational-kill-switches.md)). Полноценный per-user cost budget (агрегация стоимости из Langfuse, модель квот, мягкая деградация vs жёсткий отказ, конфигурируемость под волны) — отдельная итерация ближе к реальному продакшну (сентябрь–октябрь), backlog § Backend; стоп-кран проектируется так, чтобы не мешать ей.
- Кандидаты механики (выбор на design-brief): тумблер открытой регистрации (invite-only флаг), грубый суточный лимит ходов/запросов на пользователя, комбинация.

#### Связанное

- Backlog § Backend «Потолок затрат на пользователя» — полноценный фундамент, остаётся гейтом внешних волн.
- Backlog § Backend «Админка / менеджмент-консоль оператора» — per-user расход как админская вьюха, делается после стоп-крана.
