...
| feat-005 | ✅ Done | E2E scenarios + polish |
...
#### Состав работ
- [x] UC-1: структурирование доклада — полный флоу (создать проект → описать тему → агент использует skill "structure" → итеративная доработка → артефакт)
- [x] UC-2: research по теме — полный флоу (запрос на исследование → агент использует MCP tools для web search → структурированный результат со ссылками)
- [x] UC-3: Knowledge Sphere — персистентность контекста (работа в проекте → закрыть → вернуться → агент помнит контекст)
- [x] Фикс обнаруженных интеграционных проблем (5 багов + 1 UX polish)
- [x] Polish: критичные UX-проблемы (artifact scroll, chat title placeholder)

#### Критерии приёмки
- [x] Все три use-case проходят от начала до конца без критичных ошибок
- [x] Knowledge Sphere обновляется агентом по ходу работы и сохраняется между сессиями
- [x] Артефакты создаются и доступны для просмотра/скачивания
- [x] UI не зависает, ошибки отображаются понятно для пользователя
- [x] Приложение соответствует критерию MVP из vision.md: "не стыдно показать"

#### Артефакты
- [plan.md](iterations/integration/feat-005-e2e-polish/plan.md)
- [summary.md](iterations/integration/feat-005-e2e-polish/summary.md)
