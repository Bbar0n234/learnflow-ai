...
**Применимость к диаграммам.** Правила распространяются на содержимое Mermaid-диаграмм — текст нод, label'ы связей, легенды. Имена компонентов, методов, event'ов, констант из кода сохраняются; описательный текст в нодах подчиняется тем же критериям, что и основной markdown.
