...
Документация пишется на русском. Англицизмы — только там, где они несут смысл, которого нет в русском эквиваленте.

**Сохраняем:**
- Имена в коде, API, классах, файлах: `SecurityGuard`, `StreamEvent`, `/api/security/events`, `checkpoint`
- Устоявшиеся технические термины без точного русского эквивалента: `prompt injection`, `structured output`, `canary token`, `instruction hierarchy`, `graceful degradation`
- Имена продуктов и инструментов: `Langfuse`, `LangGraph`, `PostgreSQL`, `MCP`

**Русифицируем:**
- Составные кальки через дефис: `security-логика` → `логика защиты`, `observability-данные` → `данные наблюдаемости` или переформулировать
- Прилагательные с нормальным русским эквивалентом: `production-ready` → `готов к использованию`
- «Модные» кальки, где русский не теряет смысл: `impl-детали` → `детали реализации`, `scope-границы` → `границы скоупа`

Критерий в сомнении: читатель-не-разработчик натыкается на термин — понимает с ходу или тормозит? Тормозит, и есть русский эквивалент — русифицируй.

Примеры (плохо → хорошо):
- «API и Service Layer не содержат security-логики» → «API и Service Layer не содержат логики защиты»
- «Инкапсулирована в Agent Layer» в первом предложении архитектурного документа → вынести в подсекцию «Architecture Overview», а во вводной написать, от чего защищает система
