...
| `StreamEventMapper` | `stream_mode="updates"` → `tool_call_cancelled`. Тоже per-run — ведёт список анонсированных, но ещё не разрешённых `call_id` (`tool_result` / `artifact_created` / `artifact_updated` эмитит сам узел `tools`, повызовно, в custom-канал). |
...
2. **Артефакт — по атрибуту, не по имени.** Artifact-producing инструмент возвращает `response_format="content_and_artifact"`, где `artifact` — список элементов (один вызов может задеть несколько файлов); `kind` каждого элемента (`created`/`updated`) выбирает тип события — `artifact_created` или `artifact_updated`. Эмиссия идёт по наличию `ToolMessage.artifact`, не по whitelist имён — новый инструмент не требует правки маппера.
...
5. **Промпты — capability-язык, никогда имена инструментов.** Системный промпт любого агента и субагента говорит об инструментах только на уровне возможностей («веб-поиск», «чтение страниц») или назначения («используй доступные инструменты, когда задача этого требует») — не называет имена инструментов, провайдера или их точный состав. Конкретика (имя, параметры, когда вызывать) живёт в description самого инструмента — модель получает её оттуда, и это единственный источник. Следствие: замена MCP-провайдера промпты не трогает вовсе; в blast-radius правки промпты попадают только при изменении состава возможностей (категория появилась или исчезла) — тогда промпты всех потребителей этой возможности сверяются с фактическим toolset'ом.
