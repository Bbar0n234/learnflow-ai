...
| `artifact` | `path, title, artifact_type, kind, diff` | `ToolMessage.artifact` — по одному `artifact`-part на каждый элемент списка (джоба с N файлами → N parts); `diff: {added, removed} \| null` |
...
Артефактные parts идут одной группой в конце хода, после финального `text`, по одной карточке на путь: карточка читается как результат хода, а не как деталь того вызова, который случайно оказался пишущим, — а путь, переписанный дважды за ход (джоба перезаписала то, что раньше создал `write_file`), схлопывается в одну запись, причём `kind` не деградирует из `created` в `updated`: для читателя файл в этом ходе новый.
