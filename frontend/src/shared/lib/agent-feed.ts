...
/**
 * Редакция хода по `security_block`: вся лента схлопывается в единственный
 * текстовый элемент-заглушку — ту же форму, что отдаёт история
 ...
 * заблокированного хода в состоянии не остаётся.
 *
 * Заглушку, которую читает пользователь, рисует история: блокировка терминальна
 * и гасит живой регион вместе с лентой (`stores/stream-store.ts` → `redact`),
 * поэтому текст отсюда на экран не попадает — иначе он был бы виден дважды,
 * рядом с той же заглушкой из отрефетченной истории. Работу здесь делают оба
 * поля: `redacted: true` глушит события, приходящие после терминального, а
 * схлопнутая лента держит инвариант «live = история» на уровне состояния — что
 * бы регион ни отрисовал, показать строки заблокированного хода он не сможет.
 *
