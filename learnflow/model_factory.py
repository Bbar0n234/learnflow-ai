...
class ModelFactory:
    ...
    def create_model(self, config: ModelConfig) -> ChatOpenAI:
        ...
        logger.debug(f"Creating model with config: {config.model_name}, temp={config.temperature}, max_tokens={config.max_tokens}")
    ...
    def create_model_for_node(self, node_name: str) -> ChatOpenAI:
        ...
        if not self.config_manager.has_node_config(node_name):
            logger.warning(f"No specific configuration found for node '{node_name}', using default configuration")
