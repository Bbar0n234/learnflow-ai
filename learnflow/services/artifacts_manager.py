...
class LocalArtifactsManager:
    ...
    def _create_thread_metadata(self, thread_id: str, exam_question: str) -> Dict[str, Any]:
    ...
    def _update_thread_metadata(self, thread_path: Path, updates: Dict[str, Any]) -> None:
        ...
        # Save atomically
        self._atomic_write_file(metadata_file, json.dumps(metadata, indent=2, ensure_ascii=False))
    
    def _update_session_metadata(self, session_path: Path, updates: Dict[str, Any]) -> None:
...
"""
    ...
    async def push_learning_material(
        ...
        try:
            ...
            self._update_session_metadata(session_path, {
                "files": ["generated_material.md"]
            })
            
            logger.info(f"Successfully created learning material for thread {thread_id} session {session_id}")
    ...
    async def push_recognized_notes(
        ...
        try:
            ...
            try:
                with open(session_path / "session_metadata.json", 'r', encoding='utf-8') as f:
                    metadata = json.load(f)
                    current_files = metadata.get("files", [])
                    if "recognized_notes.md" not in current_files:
                        current_files.append("recognized_notes.md")
                    self._update_session_metadata(session_path, {"files": current_files})
            except Exception as e:
    ...
    async def push_synthesized_material(
        ...
        try:
            ...
            logger.info(f"Successfully created synthesized material for thread {thread_id}")
...
"""
            ...
            try:
                with open(session_path / "session_metadata.json", 'r', encoding='utf-8') as f:
                    ...
                    self._update_session_metadata(session_path, {"files": current_files, "status": "completed"})
            except Exception as e:
                logger.warning(f"Failed to update session metadata: {e}")
            
            logger.info(f"Successfully created questions and answers for thread {thread_id}")
    ...
    async def push_complete_materials(
        ...
        try:
            ...
            return {
                "success": True,
                "folder_path": folder_path,
                "folder_url": str((self.base_path / folder_path).absolute()),  # Local file path
                "results": results
            }
