"""HITL Manager Service for LearnFlow AI"""
...
class HITLManager:
    ...
    def is_enabled(self, node_name: str, thread_id: str) -> bool:
        ...
        Args:
            node_name: Node name
            thread_id: User identifier (from Telegram)
    ...
    def set_config(self, thread_id: str, config: HITLConfig) -> None:
        ...
        logger.info(f"Updated HITL config for thread_id: {thread_id}, config: {config.to_dict()}")
    ...
    def update_node_setting(self, thread_id: str, node_name: str, enabled: bool) -> HITLConfig:
        ...
        if hasattr(config, node_name):
            ...
            logger.info(f"Updated node '{node_name}' to {enabled} for thread_id: {thread_id}")
        else:
            logger.warning(f"Unknown node name '{node_name}' for thread_id: {thread_id}")
    ...
    def bulk_update(self, thread_id: str, enable_all: bool) -> HITLConfig:
        ...
        self.set_config(thread_id, config)
        logger.info(f"Bulk updated HITL to {'enabled' if enable_all else 'disabled'} for thread_id: {thread_id}")
